...
description: |
  搭建「个人数字分身 / 部门数字员工」聊天机器人的端到端方法论与可复用资产，从一个飞书个人数字分身（示例）的真实搭建 + 迭代过程抽象而来。
...
> 从一个飞书个人数字分身（示例）的真实搭建过程提炼。参考实现：本地 `your-digital-twin/` 项目。
...
- **阶段二 · 长出进阶能力**：后台长任务、定时晨报/周报、授权自愈、观测自愈、DM 代答等。全部配方见 [references/advanced-capabilities.md](references/advanced-capabilities.md)（从 真实迭代经验提炼，每节含反模式与血泪坑）。别一次全上，按对方的痛点排优先级（先"提速稳定"，再"后台执行器"这个地基，再定时/写作类）。
...
**进阶（文档量 > 50 篇时推荐）**：在 docs 之上叠加 LLM Wiki 编译层——让 LLM 按统一 schema 把散落原文综合成结构化实体/概念页，检索时优先命中编译产物。详见 [references/knowledge-wiki-schema.md](references/knowledge-wiki-schema.md)。
...
3. **先预研再复制**：每个对象都先跑 [[ai-agent-product-research]] 的"数字员工快速预研"——他的高频任务、要连的业务系统、要不要对访客开放、要不要定时产出，可能和已有分身完全不同，别无脑套。
...
- [references/knowledge-wiki-schema.md](references/knowledge-wiki-schema.md) — **知识库进阶：LLM Wiki 编译层搭建规范**（三层模型、frontmatter 规范、Ingest 两步思维链、检索加权）
